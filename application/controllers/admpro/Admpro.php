	...
	public function dashboard(){
			...
			if(isset($this->session->userdata['login_user'])){
					...
					if($check_user == true){	
						...
						$data['getUsersInfo'] = $getUsersInfo;	
						...
						curl_setopt($cSession1, CURLOPT_TIMEOUT_MS, 300);  
...
/*==================== Add Part Type  ==================================================================*/
	...
	public function Locks(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['subTitle'] = 'Parts > <strong>Ignition Locks</strong>';
			$config = array();
	...
	public function edit_locks($id){
		...
		if(isset($this->session->userdata['login_user'])){	
			$data['id'] = $id;
			$data['subTitle'] = 'Parts > <strong>Update Ignition Locks</strong>';	
			$data['results'] = $this->admin_model->get_part_types();
	...
	public function copy_locks($id){
		...
		if(isset($this->session->userdata['login_user'])){	
			$data['id'] = $id;
			$data['subTitle'] = 'Parts > <strong>Copy Ignition Locks</strong>';	
			$data['results'] = $this->admin_model->get_part_types();
			$data['part_info'] = $this->admin_model->get_part_info($id);
			$data["getAllMakeNames"] = $this->admin_model->getAllMakeNames();					
			$this->load->view('admin_layout/header',$data);
...
/*--------------------- Vehicle Firebase Images ----------------------------------*/
	...
	public function delete_user_feedback($id,$vid=""){
		$this->session->set_flashdata('message_display', 'Data updated successfully');
		echo $vid;
		$result = $this->admin_model->delete_user_feedback($vid);	
		redirect('admpro/user_submissions');	
	}
...
/*-------------------------- Get Firebase Updates -----------------------------*/
	...
	public function product_to_machine_purchase(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){			
			$data['subTitle'] = '<strong>Product to Machine Purchases</strong>';
			$data['results'] = get_purchase_machines();
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/product_to_machine_purchase', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function delete_product_to_machine_purchase($id){
		$result = $this->admin_model->delete_product_to_machine_purchase($id);	
		$this->session->set_flashdata('message_display', 'Data deleted successfully');
		redirect('admpro/product_to_machine_purchase');
	}
	
	public function save_product_to_machine_purchase(){
		$this->form_validation->set_rules('purchases_machine', 'Name', 'required');						
		if ($this->form_validation->run() == FALSE){
			  $this->session->set_flashdata('message_display', '<font color="#FF0000"><strong>'.validation_errors().'</strong></font>');
			  redirect('admpro/product_to_machine_purchase');		
		}else{
			$query = $this->admin_model->save_product_to_machine_purchase();
			$this->session->set_flashdata('message_display', 'Data added successfully');
			redirect('admpro/product_to_machine_purchase');
		}
	}
	...
	public function update_users_feedbacks(){
		echo $data['results'] = $this->admin_model->update_users_feedbacks();	
	}

	public function user_submissions_status(){
		$_SESSION['user_submissions_status'] = rtrim($this->input->post('status'),',');	
	}

	public function search_global_key(){
		if($this->input->post('search') == 'batteries'){
			$_SESSION['search_batteries'] = $search_key;
			$data['getAllbatteries'] = $this->admin_model->search_batteries();
			$this->load->view('admpro/battery_sorting',$data);
		}else if($this->input->post('search') == 'locks'){
			$_SESSION['search_locks'] = $search_key;
			$data['results'] = $this->admin_model->search_locks();
			$this->load->view('admpro/part_sorting',$data);
		}else if($this->input->post('search') == 'machines_info'){
			$_SESSION['search_machines_info'] = $search_key;
			$data['results'] = $this->admin_model->search_machines_info();
			$this->load->view('admpro/machine_sorting',$data);
		}else if($this->input->post('search') == 'chips'){
			$_SESSION['search_chips'] = $search_key;
			$data['getAllChips'] = $this->admin_model->search_chips();
			$this->load->view('admpro/chips_sorting',$data);
		}else if($this->input->post('search') == 'software'){
			$_SESSION['search_software'] = $search_key;
			$data['results'] = $this->admin_model->search_software();
			$this->load->view('admpro/software_sorting',$data);
		}else if($this->input->post('search') == 'accessories'){
			$_SESSION['search_accessories'] = $search_key;
			$data['results'] = $this->admin_model->search_accessories();
			$this->load->view('admpro/accessories_sorting',$data);
		}
	}
...
/*----------------------------------Tool > software -----------------------------*/
	public function software(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
		    $data['subTitle'] = 'Tools > <strong>Software</strong>';
			$config = array();
			$config["base_url"] = base_url() . "admpro/software";
			$total_row = $this->admin_model->get_software_rows();
			$config["total_rows"] = $total_row;
			//$data['getAllMakeNames'] = $this->admin_model->getAllToolsRows();
			if(isset($this->session->userdata['remotes_pagination'])){
				$per_page = $this->session->userdata['remotes_pagination']; 
			...
			$data["totalrows"] = $this->admin_model->get_software_rows();
			$data['page'] = $page;
			$data["results"] = $this->admin_model->get_software($config["per_page"],$limt_start);
			$str_links = $this->pagination->create_links();
			$data["links"] = explode('&nbsp;',$str_links );					
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/software', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function add_software(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
			$data['subTitle'] = 'Tools > <strong> Add Software</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/add_software', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function save_software(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			}else if( $this->input->post('copy_image') !=""){
				$file_name = $this->input->post('copy_image');
			}
			$query = $this->admin_model->save_software($file_name);
			$this->session->set_flashdata('message_display', 'Data added successfully');
			redirect('admpro/software');		
		}
	}
	public function edit_software($id){
		...
		if(isset($this->session->userdata['login_user'])){
			$data['id'] = $id;
			$data["software_info"] = $this->admin_model->software_info($id);
			$data['subTitle'] = 'Tools > <strong> Update Software</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/edit_software', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	public function copy_software($id){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
			$data['id'] = $id;
			$data["software_info"] = $this->admin_model->software_info($id);
			$data['subTitle'] = 'Tools > <strong> Copy Software</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/copy_software', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function update_software(){
		...
		$query = $this->admin_model->update_software($file_name);
		$this->session->set_flashdata('message_display', 'Data updated successfully');
		redirect('admpro/software');
	}
	public function delete_software($id){
		$result = $this->admin_model->delete_software($id);	
		$this->session->set_flashdata('message_display', 'Data deleted successfully');
		redirect('admpro/software');
	}
...
/*----------------------------------Tool > accessories -----------------------------*/
	public function accessories(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
		    $data['subTitle'] = 'Tools > <strong>Accessories</strong>';
			$config = array();
			$config["base_url"] = base_url() . "admpro/accessories";
			$total_row = $this->admin_model->get_accessories_rows();
			$config["total_rows"] = $total_row;
			//$data['getAllMakeNames'] = $this->admin_model->getAllToolsRows();
			if(isset($this->session->userdata['remotes_pagination'])){
				$per_page = $this->session->userdata['remotes_pagination']; 
			...
			$data["totalrows"] = $this->admin_model->get_accessories_rows();
			$data['page'] = $page;
			$data["results"] = $this->admin_model->get_accessories($config["per_page"],$limt_start);
			$str_links = $this->pagination->create_links();
			$data["links"] = explode('&nbsp;',$str_links );					
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/accessories', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function add_accessories(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
			$data['subTitle'] = 'Tools > <strong> Add Accessories</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/add_accessories', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function save_accessories(){
		...
		if ($this->form_validation->run() == FALSE){
              $this->session->set_flashdata('message_display', '<font color="#FF0000"><strong>'.validation_errors().'</strong></font>');
			  redirect('admpro/add_accessories');		
        }else{
			$file_name = "";
			if($_FILES['image']['name'] != ""){
				$pre_file_name = $_FILES['image']['name'];
			...
			$query = $this->admin_model->save_accessories($file_name);
			$this->session->set_flashdata('message_display', 'Data added successfully');
			redirect('admpro/accessories');		
		}
	}
	public function edit_accessories($id){
		...
		if(isset($this->session->userdata['login_user'])){
			$data['id'] = $id;
			$data["accessories_info"] = $this->admin_model->accessories_info($id);
			$data['subTitle'] = 'Tools > <strong> Update Accessories</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/edit_accessories', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	public function copy_accessories($id){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
			$data['id'] = $id;
			$data["accessories_info"] = $this->admin_model->accessories_info($id);
			$data['subTitle'] = 'Tools > <strong> Copy Accessories</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/copy_accessories', $data);
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	...
	public function update_accessories(){
		...
		$query = $this->admin_model->update_accessories($file_name);
		$this->session->set_flashdata('message_display', 'Data updated successfully');
		redirect('admpro/accessories');
	}
	public function delete_accessories($id){
		$result = $this->admin_model->delete_accessories($id);	
		$this->session->set_flashdata('message_display', 'Data deleted successfully');
		redirect('admpro/accessories');
	}
...
/*--------------------------------------- Software Type------------------------------------*/
	public function software_type(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['subTitle'] = ' Tools > <strong> Software Types</strong>';
			$config = array();
			$config["base_url"] = base_url() . "admpro/software_type";
			$total_row = $this->admin_model->getAllSoftwareTypeRows();
			$config["total_rows"] = $total_row;						
			$config['use_page_numbers'] = TRUE;
			$config['num_links'] = $total_row;
			$config['cur_tag_open'] = '<a class="current">';
			...
			$data["getAllSoftwareType"] = $this->admin_model->getAllSoftwareTypeDetail($config["per_page"],$limt_start);
			$str_links = $this->pagination->create_links();
			$data["links"] = explode('&nbsp;',$str_links);	
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/software_type', $data);
			$this->load->view('admin_layout/footer');
			}else{			     
				$this->load->view('admpro/index', $data);
			}	
	...
	public function add_software_type(){
	     $data =array();
	 	if(isset($this->session->userdata['login_user'])){	
			$data['subTitle'] = 'Tools > Software Types > <strong>Add New Software Type</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/add_software_type');
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}	
	public function softwaretype_add(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$add_name = $this->admin_model->Add_SoftwareType();	
			if($add_name == true){
				  $this->session->set_flashdata('message_display', 'Data added successfully');
			}else{
				$this->session->set_flashdata('message_display', 'Error in adding user! ');							 
			}							
			redirect('admpro/add_Software_type');
		}else{			     
			$this->load->view('admpro/index', $data);
		}	
	}	
	public function edit_software_type($id){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['softwareTypeId'] = $id;
			$data['subTitle'] = 'Tools > Software Types > <strong>Update Software Type</strong>';
			$data['getSoftwareTypeInfo'] = $this->admin_model->getSoftwareTypeInfo($id);
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/edit_software_type');
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}	
	...
	public function update_software_type(){
		$result = $this->admin_model->update_software_type();	
		$this->session->set_flashdata('message_display', '<div class="alert alert-success">Data updated successfully</div>');
		redirect('admpro/software_type');
	}	
	public function deletesoftwaretype($id){		
		$result = $this->admin_model->deletesoftwaretype($id);	
		if($result == 2){
			$this->session->set_flashdata('message_display', '<div class="alert alert-danger">Data cannot be deleted!</div>');
		}else{
			$this->session->set_flashdata('message_display', '<div class="alert alert-success">Data deleted successfully</div>');
...
/*--------------------------------------- Accessories Type------------------------------------*/
	public function accessories_type(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['subTitle'] = ' Tools > <strong> Accessories Types</strong>';
			$config = array();
			$config["base_url"] = base_url() . "admpro/accessories_type";
			$total_row = $this->admin_model->getAllaccessoriesTypeRows();
			$config["total_rows"] = $total_row;						
			$config['use_page_numbers'] = TRUE;
			$config['num_links'] = $total_row;
			$config['cur_tag_open'] = '<a class="current">';
			...
			$data["getAllaccessoriesType"] = $this->admin_model->getAllaccessoriesTypeDetail($config["per_page"],$limt_start);
			$str_links = $this->pagination->create_links();
			$data["links"] = explode('&nbsp;',$str_links);	
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/accessories_type', $data);
			$this->load->view('admin_layout/footer');
			}else{			     
				$this->load->view('admpro/index', $data);
			}	
	...
	public function add_accessories_type(){
	     $data =array();
	 	if(isset($this->session->userdata['login_user'])){	
			$data['subTitle'] = 'Tools > Accessories Types > <strong>Add New Accessories Type</strong>';
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/add_accessories_type');
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}	
	public function accessoriestype_add(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$add_name = $this->admin_model->Add_accessoriesType();	
			if($add_name == true){
				  $this->session->set_flashdata('message_display', 'Data added successfully');
			}else{
				$this->session->set_flashdata('message_display', 'Error in adding user! ');							 
			}							
			redirect('admpro/add_accessories_type');
		}else{			     
			$this->load->view('admpro/index', $data);
		}	
	}	
	public function edit_accessories_type($id){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['accessoriesTypeId'] = $id;
			$data['subTitle'] = 'Tools > accessories Types > <strong>Update Accessories Type</strong>';
			$data['getaccessoriesTypeInfo'] = $this->admin_model->getaccessoriesTypeInfo($id);
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/edit_accessories_type');
			$this->load->view('admin_layout/footer');
		}else{			     
			$this->load->view('admpro/index', $data);
		}	
	...
	public function update_accessories_type(){
		$result = $this->admin_model->update_accessories_type();	
		$this->session->set_flashdata('message_display', '<div class="alert alert-success">Data updated successfully</div>');
		redirect('admpro/accessories_type');
	}	
	public function deleteaccessoriestype($id){		
		$result = $this->admin_model->deleteaccessoriestype($id);	
		if($result == 2){
			$this->session->set_flashdata('message_display', '<div class="alert alert-danger">Data cannot be deleted!</div>');
		}else{
			$this->session->set_flashdata('message_display', '<div class="alert alert-success">Data deleted successfully</div>');
...
/*-------------------- EXPORT TO CSV -----------------------*/
	...
	public function export_all_vehicle_info(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
			$data['getAllVehiclesDataCSV'] = $this->admin_model->export_all_vehicle_info();
			$this->load->view('admpro/export_all_vehicle_info', $data);
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	...
	public function export_all_Keys_info(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['getAllVehiclesDataCSV'] = $this->admin_model->export_all_Keys_info();
			$this->load->view('admpro/export_all_Keys_info', $data);
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	...
	public function export_all_remotes_info(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['getAllVehiclesDataCSV'] = $this->admin_model->export_all_remotes_info();
			$this->load->view('admpro/export_all_remotes_info', $data);
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	public function import_remotes(){	
		$data =  array();
		if(isset($this->session->userdata['login_user'])){	
			$data = $this->admin_model->import_remotes();
			$data['subTitle'] = 'Parts > <strong>Import Remote to Database</strong>';	
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/import_remote_to_database', $data);
			$this->load->view('admin_layout/footer');			
	...
	public function import_key_to_database(){
		...
		if(isset($this->session->userdata['login_user'])){
			...
			$data = $this->admin_model->import_key_to_database();					
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/import_key_to_database', $data);
			$this->load->view('admin_layout/footer');			
	...
	public function aks_customers_info_ajax($part){
		$data['part'] = $part;
		$this->load->view('admpro/aks_customers_info_ajax', $data);
	}
	public function export_all_tools_info(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['getAllExportTools'] = $this->admin_model->export_all_tools_info();
			$this->load->view('admpro/export_all_tools_info', $data);
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	public function export_all_codeseries_info(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){	
			$data['getAllExportCodeSeries'] = $this->admin_model->export_all_codeseries_info();
			$this->load->view('admpro/export_all_codeseries_info', $data);
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	...
	public function quick_updates(){
		$data =  array();		
		if(isset($this->session->userdata['login_user'])){
			$data['subTitle'] = 'Quick Updates';
			$data["getAllMakeNames"] = $this->admin_model->getAllMakeNames();	
			if (isset($_POST['postdata'])) {
				$data["results"] = $this->admin_model->quick_updates_vehicles_data();
				$data["post_make_id"] = $this->input->post('make');
				$data["post_model"] = $this->input->post('Model_UUID');
			}else{
				$data["post_make_id"] = '';
				$data["post_model"] = '';
				$data["results"] = array();
			...
			$this->load->view('admin_layout/header',$data);
			$this->load->view('admpro/quick_updates', $data);
			$this->load->view('admin_layout/footer');			
	...
	public function insert_col(){
		// $col = "UPDATE t_Vehicles SET duplicated=0 WHERE id=7671";
		// if($this->db->query($col)) {
		// 	echo "t_Vehicles";
		// }
		$col2 = "ALTER TABLE t_Remotes ADD akg_num varchar(100) DEFAULT NULL";
		if($this->db->query($col2)) {
		echo "t_Remotes akg_num added ";
		}

		$col2 = "ALTER TABLE t_Keys ADD akg_num varchar(100) DEFAULT NULL";
		if($this->db->query($col2)) {
		echo "t_Keys akg_num added ";
		}
		$col2 = "ALTER TABLE t_Chips ADD akg_num varchar(100) DEFAULT NULL";
		if($this->db->query($col2)) {
		echo "t_Chips akg_num added ";
		}
		$col2 = "ALTER TABLE t_Batteries ADD akg_num varchar(100) DEFAULT NULL";
		if($this->db->query($col2)) {
		echo "t_Batteries akg_num added ";
		}
	}

	public function export_vehicle_info(){
		$data =array();
		if(isset($this->session->userdata['login_user'])){
			$data['getAllVehiclesDataCSV'] = $this->admin_model->export_all_vehicle_info();
			$this->load->view('admpro/export_vehicle_info', $data);
		}else{			     
			$this->load->view('admpro/index', $data);
		}
	}
	...
	public function getVehiclesProducts($limt_start){
        $query = $this->admin_model->getVehiclesProducts($limt_start);
		echo "<table border='1' cellpadding='10' style='white-space:nowrap;'>";
		echo "<tr>
		...
		foreach ($query as  $value) {
			$vh_id =  $value['id'];
			$data = $this->admin_model->getVehiclesProducts2($vh_id);
			$vehicle_details = $data['Make_Name'] . ' ' . $data['Model_Name'] . ' ' . str_replace(',','-',$data['Years']);
			$product_ids = array_merge(
				explode(',', $data['Mechanical_Key_Products']),
				explode(',', $data['Replacement_Blade_Products']),
				explode(',', $data['Chip_Key_Products']),
				explode(',', $data['Key_Shell_Products']),
				explode(',', $data['Transponder_Chip_Products']),
				explode(',', $data['Cloning_Chip_Products']),
				explode(',', $data['Remote_Products']),
				explode(',', $data['Remote_Shell_Products']),
				explode(',', $data['Battery_Products']),
				explode(',', $data['Emergency_Key_Products']),
				explode(',', $data['Remote_Head_Key_Products']),
				explode(',', $data['Remote_Head_Shell_Products']),
				explode(',', $data['Smart_Key_Products']),
				explode(',', $data['Smart_Key_Shell_Products']),
				explode(',', $data['Ignition_Products']),
				explode(',', $data['Door_Lock_Products']),
				explode(',', $data['Lock_Accessory_Products']),
				explode(',', $data['Lock_Products']),
				explode(',', $data['Tool_Products']),
				explode(',', $data['Software_Products']),
				explode(',', $data['Tool_Accessories_Products'])
			);
			$product_ids = array_filter($product_ids, function($value) { return !empty($value); });
			$all_product_ids = implode(',', $product_ids);
			// foreach ($product_ids as $product_id) {
			// 	//echo $vehicle_details . " | " . $product_id . "<br>";
				echo "<tr>
					<td>".$vehicle_details."</td>
					<td>".$all_product_ids."</td>
				</tr>"; 
			//}
		 }
    }
