    ...
    <div class="row">     
      <div class="col-sm-24">
        <div class="form-group addCodeSeries">          
          <a href="<?php echo adm_base_url();?>/vehicles/tip_tricks" class="btn btn-danger" >Back<<</a>        
        </div>
      </div>
    </div>
  </div>
            ...
            <?php 
                ...
                $all_vehciles = get_Firebase_vehicles_year2();
                  foreach($all_vehciles as $vehciles){
                      ...
                      $get_key_making_method = get_tip_trips_method($vehciles_id);
                      if($get_key_making_method){
                            foreach ($get_key_making_method as $methods) {
                              ...
                              if(preg_match("/\b$search\b/",$methods['Vehicle_UUID'])){
                                ...
                                $Category = str_replace('/', '_', $methods['Category']);
                                $key_making_method_array['methods'][$Category][] = array('title' => $methods['Title'],
                              ...
                              $vehicle_info_array2['tips_tricks_methods']  = $key_making_method_array;
                ...
                $cSession = curl_init();        
                curl_setopt($cSession,CURLOPT_URL,"https://autoproapp2017.firebaseio.com/vehicle1/tips_tricks_methods.json?". http_build_query($options));
