...
<div class="container-fluid">
  ...
  <?php } ?>
  <form class="site-form " method ="post" id="AddParts" action="<?php echo adm_base_url();?>/update_parts">
  <input type="hidden" name="<?php echo $this->security->get_csrf_token_name();?>" value="<?php echo $this->security->get_csrf_hash();?>" />
    <!--<h2 class="titleheadng">Add Model</h2>-->
        ...
        <div class="row">
          ...
          <div class="col-sm-10">
            <div class="inputcol">
               <input type="text" class="form-control" placeholder="Part Name" name="Part_Name" value="<?php echo $part_info[0]['Part_Name'];?>">
            </div>
          </div>
        </div>
        <div class="row">
          ...
          <div class="col-sm-10">
            <div class="inputcol">
               <select  class="form-control"  name="PartType_UUID">
               	 <option value="">Please select</option>
                <?php foreach($results as $part_type){
					if($part_info[0]['PartType_UUID'] == $part_type['UUID']){
						$selected = 'selected';
					}else{
						$selected = '';
					}
					?>
               		<option value="<?php echo $part_type['UUID'];?>" <?php echo $selected;?>> <?php echo $part_type['Name'];?></option>
                <?php } ?>
               </select>
            </div>
          </div>
        ...
        <div class="row">
          <div class=" col-sm-4 ">
            <div class="labelcol">
              <label class="control-label">Part Image Url</label>
            </div>
          </div>
          <div class="col-sm-10">
            <div class="inputcol">
               <input type="text" class="form-control" placeholder="Part Image Url" name="Part_Image_Url" value="<?php echo $part_info[0]['Part_Image_Url'];?>">
            </div>
          </div>
        </div>
        <div class="row">
          ...
          <div class="col-sm-10">
            <div class="inputcol">
               <input type="text" class="form-control" placeholder="Products" name="Products" value="<?php echo $part_info[0]['Products'];?>">
            </div>
          </div>
        </div>
        <fieldset>
                 ...
                 <select id="search" class="form-control" size="15" multiple="multiple" >
                  ...
                  foreach($get_all_vehicles as $vehicles){
                    ...
                    if( count($got_series_data) > 1){
                       ...
                       if($Code_Series_Name !=""){
                        $option_name = $get_make_nam.' '.$get_models_name.' ('.$years[0].'-'.$years[count($years)-1].') ('.$Code_Series_Name.')';
                       }else{
              ...
              <div class="inputcol">
                <label class="control-label">Selected Vehicles</label>
                 <select name="parts_vehicle[]" id="search_to" class="form-control" size="17" multiple="multiple">
                  ...
                  <?php } ?>
                   <?php
                    if($part_info[0]['Vehicles_UUID'] != ""){ 
                      $vehicles_UUIDs = explode(',', $part_info[0]['Vehicles_UUID']);
    ...
    <div class="col-md-12">
        ...
        <a href="<?php echo adm_base_url();?>/locks" class="btn btn-danger">Cancel</a>
    </div>   
  </div>
   </form>
  </div>
